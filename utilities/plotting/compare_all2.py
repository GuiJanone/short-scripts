...
def main():
    ...
    series_diff = None
    if args.diff:
        series_diff = apply_diff(energy, series)
    ...
    plot_compare_all(
        energy, series, fields, label=label,
        out=out, title=title, diff=args.diff, series_diff=series_diff
    )
